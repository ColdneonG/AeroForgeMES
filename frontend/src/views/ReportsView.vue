...
<template>
  <MesLayout active="reports-main">
    ...
    <main class="app-main" data-od-id="reports-main">
      ...
      <template v-if="activeTab==='daily'"><div class="kpi-grid mb-5"><div class="kpi-card accent-border"><span class="kpi-label">近 7 日累计产量</span><strong class="kpi-value">{{ reportedTotal.toLocaleString() }}</strong><span class="kpi-change">台 · 实时汇总</span></div><div class="kpi-card"><span class="kpi-label">近 7 日不良</span><strong class="kpi-value">{{ defectiveTotal }}</strong><span class="kpi-change">不良率 {{ defectRate }}</span></div><div class="kpi-card"><span class="kpi-label">近 7 日产值</span><strong class="kpi-value">¥{{ (reportedTotal*100).toLocaleString() }}</strong><span class="kpi-change">按出厂价 ¥100/台</span></div><div class="kpi-card"><span class="kpi-label">报工记录</span><strong class="kpi-value">{{ rows.length }}</strong><span class="kpi-change">条</span></div></div><section class="card mb-5"><div class="card-header"><h2 class="card-title">近 7 日产量趋势（按日产量汇总）</h2></div><OutputBarChart :points="trend" /></section></template>
