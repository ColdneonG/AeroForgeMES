...
import { getEquipmentRecords, maintenanceStatusLabel, runtimeStatusLabel } from '@/api/equipment'
...
const status = (row: Equipment) => String(row.equipmentStatus || 'IDLE').toUpperCase()
const isRunning = (row: Equipment) => status(row) === 'RUNNING'
const isFault = (row: Equipment) => status(row) === 'FAULT'
const isMaintenanceDue = (row: Equipment) => String(row.maintenanceStatus || 'NORMAL').toUpperCase() === 'DUE'
const filteredRows = computed(() => rows.value.filter((row) => filter.value === 'all' || (filter.value === 'running' && isRunning(row)) || (filter.value === 'fault' && isFault(row)) || (filter.value === 'idle' && !isRunning(row) && !isFault(row)) || (filter.value === 'maintenance-due' && isMaintenanceDue(row))))
...
const maintenanceDueCount = computed(() => rows.value.filter(isMaintenanceDue).length)
  ...
  <MesLayout active="equip-monitor">
    ...
    <main class="app-main">
      ...
      <section class="monitor-summary" aria-label="设备状态概览">
        ...
        <button class="summary-card maintenance" :class="{ active: filter === 'maintenance-due' }" @click="filter = 'maintenance-due'"><span class="summary-icon">⌕</span><span class="summary-content"><span class="summary-label">待保养</span><strong>{{ maintenanceDueCount }}</strong><small>已有到期保养计划</small></span><span class="summary-arrow">›</span></button>
      </section>
      <div class="data-table-wrap"><table class="data-table"><thead><tr><th>设备编码</th><th>设备名称</th><th>实时状态</th><th>保养状态</th><th>所在产线</th><th>所在工位</th><th>监控操作</th></tr></thead><tbody><tr v-if="loading"><td colspan="7">正在加载...</td></tr><tr v-else-if="!filteredRows.length"><td colspan="7">当前筛选条件下暂无设备</td></tr><tr v-for="row in filteredRows" :key="row.id"><td class="cell-mono">{{ value(row, 'equipmentCode') }}</td><td>{{ value(row, 'equipmentName') }}</td><td><span class="badge" :class="statusClass(row)"><span class="badge-dot"></span>{{ runtimeStatusLabel(row.equipmentStatus) }}</span></td><td><span class="badge" :class="isMaintenanceDue(row) ? 'badge-status-warn' : 'badge-status-ok'"><span class="badge-dot"></span>{{ maintenanceStatusLabel(row.maintenanceStatus) }}</span></td><td>{{ value(row, 'lineName') }}</td><td>{{ value(row, 'stationName') }}</td><td class="cell-actions"><button class="btn btn-ghost btn-sm" @click="changeStatus(row, 'RUNNING')">设为运行</button><button class="btn btn-ghost btn-sm" @click="changeStatus(row, 'IDLE')">设为待机</button><button class="btn btn-ghost btn-sm" @click="changeStatus(row, 'FAULT')">上报故障</button></td></tr></tbody></table></div>
    </main>
...
.monitor-summary { display:grid; grid-template-columns:repeat(5, minmax(0, 1fr)); gap:var(--space-4); margin-bottom:var(--space-5); }
...
.summary-card.total { --card-accent:var(--color-primary); }.summary-card.running { --card-accent:var(--color-success); }.summary-card.idle { --card-accent:var(--color-warning); }.summary-card.fault { --card-accent:var(--color-danger); }.summary-card.maintenance { --card-accent:var(--color-warning); }
