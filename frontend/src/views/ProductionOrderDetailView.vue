...
<template>
  <MesLayout active="order-detail">
    ...
    <main class="app-main" data-od-id="detail-main">
      ...
      <div class="tabs" data-od-id="detail-tabs"><span class="tab-item" :class="{ active: activeTab === 'overview' }" @click="activeTab = 'overview'">概览</span><span class="tab-item" :class="{ active: activeTab === 'bom' }" @click="activeTab = 'bom'">物料清单 BOM</span><span class="tab-item" :class="{ active: activeTab === 'progress' }" @click="activeTab = 'progress'">工序进度</span><span class="tab-item" :class="{ active: activeTab === 'quality' }" @click="activeTab = 'quality'">质量记录</span></div>
      ...
      <section v-show="activeTab === 'bom'" class="card text-muted">BOM 数据模型与接口尚未建设，当前不展示模拟物料清单。</section>
