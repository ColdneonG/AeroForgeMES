...
const progress = computed(() => {
  ...
  return planned ? Math.min(100, Number(order.value?.completedQty || 0) / planned * 100) : 0
})
function operationProgress(row: OperationProgress) {
  const planned = Number(row.planQty || 0)
  return planned ? Math.min(100, Number(row.reportedQty || 0) / planned * 100) : 0
}
